...
const TrainingStats = ({ single }) => {
  ...
  const setData = useCallback(function (querySnapshot, snapshot) {
    ...
    querySnapshot.forEach((doc) => {
      ...
      console.log(
        realData.lightDotsClicked,
        realData.splitted[0] - realData.lightDotsClicked,
        realData.splitted[0]
      );

      datasetsData[0].data.push(
        (realData.lightDotsClicked / realData.splitted[0]) * 100
      );
      datasetsData[1].data.push(
        (realData.optimalDotsClicked / realData.splitted[1]) * 100
      );
      datasetsData[2].data.push(
        (realData.brightDotsClicked / realData.splitted[2]) * 100
      );
